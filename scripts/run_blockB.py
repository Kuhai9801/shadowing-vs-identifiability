"""Run Block B experiments (digital -> analog crossover) and write results.

Block B specification (from the plan)
------------------------------------
  * Matrix: A = A1 (canonical).
  * Noise: sigma = 1e-3.
  * Gap lengths: q in {4,5,6,7,8,9,10}.
  * Instances per q: N_inst = 20.
  * Hypotheses per instance: K_max = 64.
  * Solvers: Neural-DA (Newton optional; disabled by default).

Outputs
-------
Writes CSV files analogous to Block A:
  * blockB_hypotheses.csv
  * blockB_success.csv
  * blockB_summary.csv
Also writes:
  * blockB_meta.json
...
from scripts.common import compute_global_epsilon_embedding_x, default_K_list, parse_int_list
...
def _run_instances(
    *,
    ...
    for j_inst, (inst_id, inst) in enumerate(instances_with_ids):
        ...
        #
        # These are distinct objects and are reported separately.
        ...
        for K in K_list:
            ...
            # (A) Imputation success (x-only): min e_impute_x within the top-K score-ranked candidates.
            # eps_success is computed in embedding-x units.
        ...
        if progress_q is not None:
            ...
            print(
                ...
                f"q={inst.q} | {t1 - t0:.2f}s (batch_elapsed={elapsed/60.0:.1f} min)"
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Block B (digital -> analog crossover) experiments.")
    parser.add_argument("--outdir", type=str, default="6_results", help="Output directory.")
    parser.add_argument("--seed", type=int, default=23456, help="Base seed.")
    parser.add_argument("--N_inst", type=int, default=20, help="Instances per q.")
    ...
    parser.add_argument("--sigma", type=float, default=1e-3, help="Noise bound sigma.")
    parser.add_argument(
        "--q_list",
        type=str,
        default=None,
        help=(
            "Comma-separated list of gap lengths q to run (overrides the default 4,5,6,7,8,9,10). "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Observation admissibility ball radius in embedding-x (chordal) units per endpoint. "
            "Default: derived exactly from sigma as 2*sin(pi*sigma), matching d_T1(x, x_obs)<=sigma. "
            "Set to 0 to enforce exact endpoint matching (legacy)."
        ),
    )
    parser.add_argument("--device", type=str, default=None, help="Torch device override, e.g. cuda or cpu.")
    parser.add_argument("--with_newton", action="store_true", help="Also run Newton baseline (optional).")
    parser.add_argument(
        "--torch_deterministic",
        action="store_true",
        help="Enable torch deterministic algorithms (may reduce performance).",
    )
    ...
    seed_everything(int(args.seed), deterministic_torch=bool(args.torch_deterministic))
    ...
    # Endpoint admissibility: by default allow the optimizer to choose x_0 and x_q within the
    # sigma-noise balls around the observed endpoints, consistent with Definition I.1.
    # This is implemented strictly in embedding space via a chordal-distance ball on E_x.
    if args.obs_ball_radius is None:
        ncfg.obs_ball_radius = float(embedding_circle_distance_from_torus_distance(float(sigma)))
    else:
        ncfg.obs_ball_radius = float(args.obs_ball_radius)
    ...
    if num_workers == 1:
        hyp_rows, succ_rows = _run_instances(
            ...
            K_list=K_list,
            eps_success=eps_success,
            with_newton=bool(args.with_newton),
            progress_q=None,
            quiet=args.quiet,
        )
        ...
        # Split instances into roughly-equal chunks.
        chunks: List[List[Tuple[int, Any]]] = [[] for _ in range(num_workers)]
        ...
        for w in range(num_workers):
            ...
            out_h = os.path.join(args.outdir, f"_blockB_hypotheses_worker{w}.csv")
            out_s = os.path.join(args.outdir, f"_blockB_success_worker{w}.csv")
        ...
        if (not args.quiet) and (tqdm is not None):
            pbar = tqdm(total=total_instances, desc="Block B instances", unit="inst")
        ...
        # Optional cleanup of worker shards.
    ...
    # Write outputs.
    df_h.to_csv(os.path.join(args.outdir, "blockB_hypotheses.csv"), index=False)
    df_s.to_csv(os.path.join(args.outdir, "blockB_success.csv"), index=False)

    # Aggregate *coverage* success probabilities.
    # Primary coverage success = symbolic itinerary match (d_H==0) within the top-K score-ranked hypotheses.
    ...
    df_summary.to_csv(os.path.join(args.outdir, "blockB_summary.csv"), index=False)

    # Convenience: write the per-instance *single-output* table (K=1 rows).
    df_single = df_s[df_s["K"] == 1].copy()
    if not df_single.empty:
        df_single.to_csv(os.path.join(args.outdir, "blockB_single.csv"), index=False)
        ...
        df_single_summary.to_csv(os.path.join(args.outdir, "blockB_single_summary.csv"), index=False)
    ...
    with open(os.path.join(args.outdir, "blockB_meta.json"), "w", encoding="utf-8") as f:
